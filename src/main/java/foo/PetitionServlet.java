...
@WebServlet(name = "PetServlet", urlPatterns = { "/petitionServlet" })
		...
		for (int i = 0; i < 30; i++) {
			for (int j = 0; j < 10; j++) {
				...
				for (int k = 0; k < nbMaxPetition; k++) {
					...
					p.setProperty("theme", + j);
					p.setProperty("titre",+ j);
					p.setProperty("description", + j);
					p.setProperty("date", dateValue);
					p.setProperty("proprietaire",  userId);
                    p.setProperty("nomProprietaire",  j);
